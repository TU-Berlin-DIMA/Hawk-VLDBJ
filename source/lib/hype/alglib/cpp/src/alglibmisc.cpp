...
_hqrndstate_owner::_hqrndstate_owner() {
  p_struct = (alglib_impl::hqrndstate *)alglib_impl::ae_malloc(
      sizeof(alglib_impl::hqrndstate), NULL);
...
_hqrndstate_owner::_hqrndstate_owner(const _hqrndstate_owner &rhs) {
  ...
  alglib_impl::_hqrndstate_init_copy(
      p_struct, const_cast<alglib_impl::hqrndstate *>(rhs.p_struct), NULL);
}
...
_kdtree_owner::_kdtree_owner() {
  p_struct = (alglib_impl::kdtree *)alglib_impl::ae_malloc(
      sizeof(alglib_impl::kdtree), NULL);
  if (p_struct == NULL) throw ap_error("ALGLIB: malloc error");
...
_kdtree_owner::_kdtree_owner(const _kdtree_owner &rhs) {
  ...
  alglib_impl::_kdtree_init_copy(
      p_struct, const_cast<alglib_impl::kdtree *>(rhs.p_struct), NULL);
}
...
void kdtreebuildtagged(const real_2d_array &xy, const integer_1d_array &tags,
  ...
  if ((xy.rows() != tags.length()))
    throw ap_error(
        "Error while calling 'kdtreebuildtagged': looks like one of arguments "
        "has wrong size");
  n = xy.rows();
  alglib_impl::ae_state_init(&_alglib_env_state);
  try {
...
_xdebugrecord1_owner::_xdebugrecord1_owner() {
  p_struct = (alglib_impl::xdebugrecord1 *)alglib_impl::ae_malloc(
      sizeof(alglib_impl::xdebugrecord1), NULL);
  if (p_struct == NULL) throw ap_error("ALGLIB: malloc error");
...
_xdebugrecord1_owner::_xdebugrecord1_owner(const _xdebugrecord1_owner &rhs) {
  ...
  alglib_impl::_xdebugrecord1_init_copy(
      p_struct, const_cast<alglib_impl::xdebugrecord1 *>(rhs.p_struct), NULL);
}
...
Array is passed using "out" convention.
  ...
  -- ALGLIB --
     Copyright 11.10.2013 by Bochkanov Sergey
*************************************************************************/
void xdebugc2outsincos(const ae_int_t m, const ae_int_t n,
                       complex_2d_array &a) {
  alglib_impl::ae_state _alglib_env_state;
  alglib_impl::ae_state_init(&_alglib_env_state);
  try {
...
static void nearestneighbor_kdtreequerynnrec(kdtree *kdt, ae_int_t offs,
                                             ae_state *_state);
...
static void nearestneighbor_kdtreeallocdatasetdependent(kdtree *kdt, ae_int_t n,
                                                        ...
                                                        ae_int_t ny,
                                                        ae_state *_state);
static void nearestneighbor_kdtreealloctemporaries(kdtree *kdt, ae_int_t n,
                                                   ae_int_t nx, ae_int_t ny,
                                                   ae_state *_state);
...
void kdtreebuildtagged(/* Real    */ ae_matrix *xy,
   ...
   */
  maxnodes = n;
  ae_vector_set_length(&kdt->nodes,
                       nearestneighbor_splitnodesize * 2 * maxnodes, _state);
  ae_vector_set_length(&kdt->splits, 2 * maxnodes, _state);
  nodesoffs = 0;
  splitsoffs = 0;
  ae_v_move(&kdt->curboxmin.ptr.p_double[0], 1, &kdt->boxmin.ptr.p_double[0], 1,
            ae_v_len(0, nx - 1));
  ae_v_move(&kdt->curboxmax.ptr.p_double[0], 1, &kdt->boxmax.ptr.p_double[0], 1,
            ae_v_len(0, nx - 1));
...
ae_int_t kdtreequeryaknn(kdtree *kdt,
  ...
  ae_assert(ae_fp_greater_eq(eps, (double)(0)),
            "KDTreeQueryAKNN: incorrect Eps!", _state);
  ae_assert(x->cnt >= kdt->nx, "KDTreeQueryAKNN: Length(X)<NX!", _state);
  ae_assert(isfinitevector(x, kdt->nx, _state),
            "KDTreeQueryAKNN: X contains infinite or NaN values!", _state);
...
void kdtreequeryresultsx(kdtree *kdt,
  ...
  for (i = 0; i <= k - 1; i++) {
    ae_v_move(&x->ptr.pp_double[i][0], 1,
              &kdt->xy.ptr.pp_double[kdt->idx.ptr.p_int[i]][kdt->nx], 1,
              ae_v_len(0, kdt->nx - 1));
  }
}
...
void kdtreequeryresultsdistances(kdtree *kdt,
  ...
  if (kdt->normtype == 2) {
    for (i = 0; i <= k - 1; i++) {
      r->ptr.p_double[i] =
          ae_sqrt(ae_fabs(kdt->r.ptr.p_double[i], _state), _state);
    }
  }
}
...
void kdtreeunserialize(ae_serializer *s, kdtree *tree, ae_state *_state) {
  ...
  nearestneighbor_kdtreealloctemporaries(tree, tree->n, tree->nx, tree->ny,
                                         _state);
}
...
static void nearestneighbor_kdtreegeneratetreerec(
  ...
  ae_v_move(&kdt->buf.ptr.p_double[0], 1, &kdt->xy.ptr.pp_double[i1][d],
            kdt->xy.stride, ae_v_len(0, i2 - i1 - 1));
  n = i2 - i1;
  cntless = 0;
  cntgreater = 0;
...
static void nearestneighbor_kdtreequerynnrec(kdtree *kdt, ae_int_t offs,
  ...
  if (kdt->nodes.ptr.p_int[offs] > 0) {
    ...
    for (i = i1; i <= i2 - 1; i++) {
      ...
      if (kdt->normtype == 0) {
        for (j = 0; j <= nx - 1; j++) {
          ptdist = ae_maxreal(ptdist, ae_fabs(kdt->xy.ptr.pp_double[i][j] -
                                                  kdt->x.ptr.p_double[j],
                                              _state),
                              _state);
        }
      }
      if (kdt->normtype == 1) {
        for (j = 0; j <= nx - 1; j++) {
          ptdist = ptdist +
                   ae_fabs(kdt->xy.ptr.pp_double[i][j] - kdt->x.ptr.p_double[j],
                           _state);
        }
      }
      if (kdt->normtype == 2) {
        for (j = 0; j <= nx - 1; j++) {
          ptdist = ptdist +
                   ae_sqr(kdt->xy.ptr.pp_double[i][j] - kdt->x.ptr.p_double[j],
                          _state);
        }
      }
       ...
       */
      if (ae_fp_eq(kdt->rneeded, (double)(0)) ||
          ae_fp_less_eq(ptdist, kdt->rneeded)) {
        /*
    ...
    for (i = 0; i <= 1; i++) {
      ...
      if (updatemin) {
        ...
        if (ae_fp_less_eq(t1, s)) {
          ...
          if (kdt->normtype == 1) {
            kdt->curdist =
                kdt->curdist - ae_maxreal(v - t1, (double)(0), _state) + s - t1;
          }
          if (kdt->normtype == 2) {
            kdt->curdist =
                kdt->curdist -
        ...
        if (ae_fp_greater_eq(t1, s)) {
          ...
          if (kdt->normtype == 1) {
            kdt->curdist =
                kdt->curdist - ae_maxreal(t1 - v, (double)(0), _state) + t1 - s;
          }
          if (kdt->normtype == 2) {
            kdt->curdist =
                kdt->curdist -
       ...
       */
      if (ae_fp_neq(kdt->rneeded, (double)(0)) &&
          ae_fp_greater(kdt->curdist, kdt->rneeded)) {
        todive = ae_false;
      } else {
        if (kdt->kcur < kdt->kneeded || kdt->kneeded == 0) {
          /*
           ...
           */
          todive = ae_fp_less_eq(kdt->curdist,
                                 kdt->r.ptr.p_double[0] * kdt->approxf);
        }
      }
      if (todive) {
