...
#pragma unroll
  ...
  */
  __device__ void insertEntry(HashTable& ht, HtUnsigned key, HtUnsigned value,
    ...
    for (int its = 1; its <= ht.maxInsertIterations; its++) {
      *(unsigned long long*)&entry =
          atomicExch((unsigned long long*)&ht.table[location],
                     *(unsigned long long*)&entry);
    ...
    if (entry.key != kEmpty) {
      ...
      *(unsigned long long*)&replaced_entry =
          atomicCAS((unsigned long long*)(stash + slot), kEntryEmpty,
                    *(unsigned long long*)&entry);
