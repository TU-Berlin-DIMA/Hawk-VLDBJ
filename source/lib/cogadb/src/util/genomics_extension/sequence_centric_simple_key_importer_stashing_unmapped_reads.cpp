...
bool SequenceCentric_SimpleKey_WithStash_Importer::_createDatabase() {
  ...
  if (this->compress_data) {
    ...
    // unmapped reads
    ur_id_column = OIDTypedColumnPtr(
        new VoidCompressedColumnNumber<TID>(UR_ID_COL_NAME, OID));
    ur_qname_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            UR_QNAME_COL_NAME, VARCHAR));
    ur_flag_column =
        IntTypedColumnPtr(new BitPackedDictionaryCompressedColumn<int>(
            UR_FLAG_COL_NAME, INT, 16));
    ur_c_id_column =
        OIDTypedColumnPtr(new RLECompressedColumn<TID>(UR_C_ID_COL_NAME, OID));
    ur_pos_column = IntTypedColumnPtr(new Column<int>(UR_POS_COL_NAME, INT));
    ur_map_qual_column = IntTypedColumnPtr(
        new RLECompressedColumn<int>(UR_MAPPING_QUALITY_COL_NAME, INT));
    ur_cigar_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            UR_CIGAR_COL_NAME, VARCHAR));
    ur_seq_column = StringTypedColumnPtr(
        new DictionaryCompressedColumn<std::string>(UR_SEQ_COL_NAME, VARCHAR));
    ur_qual_column = StringTypedColumnPtr(
        new DictionaryCompressedColumn<std::string>(UR_QUAL_COL_NAME, VARCHAR));
    ur_sg_id_column = OIDTypedColumnPtr(
        new RLECompressedColumn<TID>(UR_SAMPLE_GENOME_ID_COL_NAME, OID));
    // FIXME mate information - no compression yet
    ur_mate_id_column =
        OIDTypedColumnPtr(new Column<TID>(UR_MATE_ID_COL_NAME, OID));
    ...
    uread_aux_id_column = OIDTypedColumnPtr(
        new VoidCompressedColumnNumber<TID>(URAUX_ID_COL_NAME, OID));
    uread_aux_read_id_column = OIDTypedColumnPtr(
        new RLECompressedColumn<TID>(URAUX_READ_ID_COL_NAME, OID));
    // 128 unique tags
    uread_aux_tag_column = StringTypedColumnPtr(
        new BitPackedDictionaryCompressedColumn<std::string>(URAUX_TAG_COL_NAME,
                                                             VARCHAR, 8));
    // 16 possible types
    uread_aux_type_column = StringTypedColumnPtr(
        new BitPackedDictionaryCompressedColumn<std::string>(
            URAUX_TYPE_COL_NAME, VARCHAR, 4));
    uread_aux_value_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            URAUX_VALUE_COL_NAME, VARCHAR));
  } else {
    // #########################
    // # reference genome data #
    // #########################
    ...
    // unmapped reads
    ur_id_column = OIDTypedColumnPtr(new Column<TID>(UR_ID_COL_NAME, OID));
    ur_qname_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            UR_QNAME_COL_NAME, VARCHAR));
    ur_flag_column = IntTypedColumnPtr(new Column<int>(UR_FLAG_COL_NAME, INT));
    ur_c_id_column = OIDTypedColumnPtr(new Column<TID>(UR_C_ID_COL_NAME, OID));
    ur_pos_column = IntTypedColumnPtr(new Column<int>(UR_POS_COL_NAME, INT));
    ur_map_qual_column =
        IntTypedColumnPtr(new Column<int>(UR_MAPPING_QUALITY_COL_NAME, INT));
    ur_cigar_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            UR_CIGAR_COL_NAME, VARCHAR));
    ur_seq_column = StringTypedColumnPtr(
    ...
    ur_sg_id_column =
        OIDTypedColumnPtr(new Column<TID>(UR_SAMPLE_GENOME_ID_COL_NAME, OID));
    // FIXME mate information - no compression yet
    ur_mate_id_column =
        OIDTypedColumnPtr(new Column<TID>(UR_MATE_ID_COL_NAME, OID));
    ...
    uread_aux_id_column =
        OIDTypedColumnPtr(new Column<TID>(URAUX_ID_COL_NAME, OID));
    uread_aux_read_id_column =
        OIDTypedColumnPtr(new Column<TID>(URAUX_READ_ID_COL_NAME, OID));
    uread_aux_tag_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            URAUX_TAG_COL_NAME, VARCHAR));
    uread_aux_type_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            URAUX_TYPE_COL_NAME, VARCHAR));
    uread_aux_value_column =
        StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(
            URAUX_VALUE_COL_NAME, VARCHAR));
  }
  ...
  vector<ColumnPtr> uread_cols;
  vector<ColumnPtr> read_aux_cols;
  vector<ColumnPtr> uread_aux_cols;
  // reference genome
  ...
  // FIXME disabled as mapped and unmapped reads can be in one pair
  //        if (!readTable->setForeignKeyConstraint(R_MATE_ID_COL_NAME,
  ...
  // unmapped reads
  out << "Creating " << UR_TBL_NAME << " table ..." << endl;
  uread_cols.push_back(ur_id_column);
  uread_cols.push_back(ur_qname_column);
  uread_cols.push_back(ur_flag_column);
  uread_cols.push_back(ur_c_id_column);
  uread_cols.push_back(ur_pos_column);
  uread_cols.push_back(ur_map_qual_column);
  uread_cols.push_back(ur_cigar_column);
  uread_cols.push_back(ur_seq_column);
  uread_cols.push_back(ur_qual_column);
  uread_cols.push_back(ur_sg_id_column);
  uread_cols.push_back(ur_mate_id_column);

  unmappedReadsTable = TablePtr(new Table(UR_TBL_NAME, uread_cols));
  if (!unmappedReadsTable->setPrimaryKeyConstraint(UR_ID_COL_NAME)) {
    COGADB_ERROR("Failed to set Primary Key Constraint!", "");
    return false;
  }
  if (!unmappedReadsTable->setForeignKeyConstraint(
          UR_SAMPLE_GENOME_ID_COL_NAME, SG_ID_COL_NAME, SG_TBL_NAME)) {
    COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
    return false;
  }
  if (!unmappedReadsTable->setForeignKeyConstraint(UR_C_ID_COL_NAME,
                                                   C_ID_COL_NAME, C_TBL_NAME)) {
    COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
    return false;
  }
  // add read table before we can set a foreign key
  addToGlobalTableList(unmappedReadsTable);
  // FIXME disabled as mapped and unmapped reads can be in one pair
  //        if
  //        (!unmappedReadsTable->setForeignKeyConstraint(UR_MATE_ID_COL_NAME,
  //        R_ID_COL_NAME, R_TBL_NAME)) {
  //            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
  //            return false;
  //        }
  ...
  // read aux
  out << "Creating " << URAUX_TBL_NAME << " table ..." << endl;
  uread_aux_cols.push_back(uread_aux_id_column);
  uread_aux_cols.push_back(uread_aux_read_id_column);
  uread_aux_cols.push_back(uread_aux_tag_column);
  uread_aux_cols.push_back(uread_aux_type_column);
  uread_aux_cols.push_back(uread_aux_value_column);
  unmappedReadsAuxTable = TablePtr(new Table(URAUX_TBL_NAME, uread_aux_cols));
  if (!unmappedReadsAuxTable->setPrimaryKeyConstraint(URAUX_ID_COL_NAME)) {
    COGADB_ERROR("Failed to set Primary Key Constraint!", "");
    return false;
  }
  if (!unmappedReadsAuxTable->setForeignKeyConstraint(
          URAUX_READ_ID_COL_NAME, UR_ID_COL_NAME, UR_TBL_NAME)) {
    COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
    return false;
  }
  addToGlobalTableList(unmappedReadsAuxTable);
  ...
  storeTable(unmappedReadsTable);
  storeTable(readAuxTable);
  storeTable(unmappedReadsAuxTable);
  return true;
}

bool SequenceCentric_SimpleKey_WithStash_Importer::_initDatabaseConnection() {
  ...
  for (std::vector<TablePtr>::const_iterator table = tables.begin();
    ...
    if ((*table)->getName().compare(UR_TBL_NAME) == 0) {
      unmappedReadsTable = *table;
      existing_tables++;
      continue;
    }
    ...
    if ((*table)->getName().compare(URAUX_TBL_NAME) == 0) {
      unmappedReadsAuxTable = *table;
      existing_tables++;
      continue;
    }
  }
  if (existing_tables == 7) {
    ...
    ur_id_column = boost::dynamic_pointer_cast<OIDTypedColumnType>(
        readTable->getColumnbyName(UR_ID_COL_NAME));
    assert(ur_id_column != NULL);
    ur_qname_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readTable->getColumnbyName(UR_QNAME_COL_NAME));
    assert(ur_qname_column != NULL);
    ur_flag_column = boost::dynamic_pointer_cast<IntTypedColumnType>(
        readTable->getColumnbyName(UR_FLAG_COL_NAME));
    assert(ur_flag_column != NULL);
    ur_c_id_column = boost::dynamic_pointer_cast<OIDTypedColumnType>(
        readTable->getColumnbyName(UR_C_ID_COL_NAME));
    assert(ur_flag_column != NULL);
    ur_pos_column = boost::dynamic_pointer_cast<IntTypedColumnType>(
        readTable->getColumnbyName(UR_POS_COL_NAME));
    assert(ur_flag_column != NULL);
    ur_map_qual_column = boost::dynamic_pointer_cast<IntTypedColumnType>(
        readTable->getColumnbyName(UR_MAPPING_QUALITY_COL_NAME));
    assert(ur_map_qual_column != NULL);
    ur_cigar_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readTable->getColumnbyName(UR_CIGAR_COL_NAME));
    assert(ur_cigar_column != NULL);
    ur_seq_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readTable->getColumnbyName(UR_SEQ_COL_NAME));
    assert(ur_seq_column != NULL);
    ur_qual_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readTable->getColumnbyName(UR_QUAL_COL_NAME));
    assert(ur_qual_column != NULL);
    ur_sg_id_column = boost::dynamic_pointer_cast<OIDTypedColumnType>(
        readTable->getColumnbyName(UR_SAMPLE_GENOME_ID_COL_NAME));
    assert(ur_sg_id_column != NULL);
    ur_mate_id_column = boost::dynamic_pointer_cast<OIDTypedColumnType>(
        readTable->getColumnbyName(UR_MATE_ID_COL_NAME));
    assert(ur_mate_id_column != NULL);
    ...
    uread_aux_id_column = boost::dynamic_pointer_cast<OIDTypedColumnType>(
        readAuxTable->getColumnbyName(URAUX_ID_COL_NAME));
    assert(uread_aux_id_column != NULL);
    uread_aux_read_id_column = boost::dynamic_pointer_cast<OIDTypedColumnType>(
        readAuxTable->getColumnbyName(URAUX_READ_ID_COL_NAME));
    assert(uread_aux_read_id_column != NULL);
    uread_aux_tag_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readAuxTable->getColumnbyName(URAUX_TAG_COL_NAME));
    assert(uread_aux_tag_column != NULL);
    uread_aux_type_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readAuxTable->getColumnbyName(URAUX_TYPE_COL_NAME));
    assert(uread_aux_type_column != NULL);
    uread_aux_value_column = boost::dynamic_pointer_cast<StringTypedColumnType>(
        readAuxTable->getColumnbyName(URAUX_VALUE_COL_NAME));
    assert(uread_aux_value_column != NULL);
...
SequenceCentric_SimpleKey_WithStash_Importer::
    SequenceCentric_SimpleKey_WithStash_Importer(ostream &_out,
                                                 bool _compress_data,
                                                 bool _verbose)
...
bool SequenceCentric_SimpleKey_WithStash_Importer::_importReferenceGenomeData(
...
bool SequenceCentric_SimpleKey_WithStash_Importer::importReferenceGenomeData(
...
SequenceCentric_SimpleKey_WithStash_Importer::getAuxiliaryStringVector(
    bam1_t *alignment) {
  ...
  for (int j = 0; j < alignment->l_aux;) {
    ...
    switch (type) {
      ...
      case 'c':
      case 'A':
        value_string << static_cast<int8_t>(auxiliary_info[j]);
        j += 1;
        type_string = "A";
        break;
      case 'S':
...
*/
bool SequenceCentric_SimpleKey_WithStash_Importer::_importUnalignedAlignment(
    TID sample_genome_id, TID id_of_contig, bam1_t *alignment,
    const bam1_core_t c, TID uread_id, TID mate_id, TID &uraux_id,
  ...
  ur_id_column->insert(uread_id);
  ur_qname_column->insert(string(bam1_qname(alignment)));
  ur_flag_column->insert(static_cast<int>(c.flag));
  ur_c_id_column->insert(id_of_contig);
  ur_pos_column->insert(pos);
  ur_map_qual_column->insert(static_cast<int>(c.qual));
  ur_cigar_column->insert(converted_cigar);
  ...
  ur_seq_column->insert(seq_string.str());
  ...
  ur_qual_column->insert(qual_string.str());
  ur_sg_id_column->insert(sample_genome_id);
  ur_mate_id_column->insert(mate_id);
  ...
  for (aux_it = aux_infos.begin(); aux_it != aux_infos.end(); aux_it++) {
    uread_aux_id_column->insert(uraux_id);
    uread_aux_read_id_column->insert(uread_id);
    uread_aux_tag_column->insert(aux_it->tag);
    uread_aux_type_column->insert(aux_it->type);
    uread_aux_value_column->insert(aux_it->value);
    uraux_id++;
  }
  return true;
}
...
* int* numberOfReads ... for statistics
*/
bool SequenceCentric_SimpleKey_WithStash_Importer::_importAlignment(
    TID sample_genome_id, TID id_of_contig, bam1_t *alignment,
...
string SequenceCentric_SimpleKey_WithStash_Importer::convertCIGAR2String(
...
bool SequenceCentric_SimpleKey_WithStash_Importer::_importSampleGenomeData(
  ...
  TID uread_id = unmappedReadsTable->getNumberofRows();
  TID raux_id = readAuxTable->getNumberofRows();
  TID uraux_id = unmappedReadsAuxTable->getNumberofRows();
  ...
  //                    SelectionParam(processorSpecification,
  //                    ValueConstantPredicate, reference_genome_id, EQUAL));
  //
  //            if (contig_TIDs->size() == 0) {
  //                out << "ERROR: Couldn't find contigs for " <<
  //                reference_genome_id << "." << endl;
  //                return false;
  //            }
  //
  //            ColumnPtr selected_contig_ids = c_id_column->gather(contig_TIDs,
  //            GatherParam(processorSpecification));
  //            ColumnPtr selected_contig_names =
  //            c_name_column->gather(contig_TIDs,
  //            GatherParam(processorSpecification));
  //
  ...
  //            if (selected_contig_ids_typed->getColumnType() !=
  //            PLAIN_MATERIALIZED) {
  //                queried_contig_ids =
  //                selected_contig_ids_typed->copyIntoDenseValueColumn(processorSpecification);
  //            } else {
  //                queried_contig_ids = boost::dynamic_pointer_cast<Column<TID>
  //                >(selected_contig_ids);
  ...
  //            selected_contig_names_typed->copyIntoDenseValueColumn(processorSpecification);
  //
  ...
  //                contigName2IdMap.insert(make_pair(contig_names[i],
  //                contig_ids_[i]));
  //            }
  //        }
...
* first read through the complete file and count the alignment */
  ...
  TID id_of_contig = uint64_max;
  TID id_of_contig_mate = uint64_max;
  ...
  while (alignmentFile->getNextAlignment(loop_alignment) >= 0) {
    ...
    if (mate_iterator != mate_map.end()) {
      ...
      TID import_mate_id = uint64_max;
      bool isMateAligned = false;
      if ((mate_core.flag & BAM_FUNMAP) == 0) {
        // TODO if chromosome id not set (< 0 or > n_targets) flag seems to be
        // corrupted!
        id_of_contig_mate = chromosome_id_mate;
        import_mate_id = read_id;
        read_id++;
        isMateAligned = true;
      } else {
        if (chromosome_id_mate >= 0 &&
            chromosome_id_mate < alignmentFile->getHeader()->n_targets) {
          id_of_contig = chromosome_id_mate;
        } else {
          id_of_contig_mate = uint64_max;
        }
        import_mate_id = uread_id;
        uread_id++;
        isMateAligned = false;
      }
      TID import_read_id = uint64_max;
      bool isAligned = false;
      if ((core.flag & BAM_FUNMAP) == 0) {
        // TODO if chromosome id not set (< 0 or > n_targets) flag seems to be
        // corrupted!
        id_of_contig = chromosome_id;
        ...
        if (chromosome_id >= 0 &&
          ...
          id_of_contig = uint64_max;
        }
        import_read_id = uread_id;
        uread_id++;
        isAligned = false;
      }
      ...
      if (isMateAligned) {
        ...
        _importUnalignedAlignment(sample_genome_id, id_of_contig_mate,
                                  alignment_mate, mate_core, import_mate_id,
                                  import_read_id, uraux_id, converted_cigar);
        ++numberOfUnmappedReads;
      }

      cigar = bam1_cigar(alignment);
      /* do CIGAR conversion */
      converted_cigar = convertCIGAR2String(cigar, core.n_cigar);
      if (isAligned) {
        ...
        // unmapped read
        _importUnalignedAlignment(sample_genome_id, id_of_contig, alignment,
                                  core, import_read_id, import_mate_id,
                                  uraux_id, converted_cigar);
        ++numberOfUnmappedReads;
      }
  ...
  for (mate_map_iterator_type read_iterator = mate_map.begin();
    ...
    TID import_id = std::numeric_limits<uint64_t>::max();
    bool isAligned = false;
    if ((nomate_core.flag & BAM_FUNMAP) == 0) {
      // TODO if chromosome id not set (< 0 or > n_targets) flag seems to be
      // corrupted!
      id_of_contig = static_cast<TID>(nomate_core.tid);
      import_id = read_id;
      read_id++;
      isAligned = true;
    } else {
      if (nomate_core.tid >= 0 &&
          nomate_core.tid < alignmentFile->getHeader()->n_targets) {
        id_of_contig = static_cast<TID>(nomate_core.tid);
      } else {
        id_of_contig = std::numeric_limits<uint64_t>::max();
      }
      import_id = uread_id;
      uread_id++;
      isAligned = false;
    ...
    if (isAligned) {
      ...
      // unmapped read
      _importUnalignedAlignment(sample_genome_id, id_of_contig,
  ...
  unmappedReadsTable->setNumberofRows(uread_id);
  readAuxTable->setNumberofRows(raux_id);
  unmappedReadsAuxTable->setNumberofRows(uraux_id);
  end_importing = getTimestamp() - start;
  start = getTimestamp();
...
bool SequenceCentric_SimpleKey_WithStash_Importer::importSampleGenomeData(
