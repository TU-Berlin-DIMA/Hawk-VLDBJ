...
namespace CudaHT {
  namespace CuckooHashing {
    ...
    /*! @param[in]  N           Number of hash functions.
        ...
        @param[in]  d_keys      Debug only: Device memory array containing the
       input keys.
    ...
    template <unsigned N>
    struct Functions {
      //! The constants required for all of the hash functions, including the
      // stash.  Each function requires 2.
      uint2 constants[N];
    ...
    inline __device__ __host__ unsigned hash_function_inner(
        const uint2 constants, const unsigned key) {
#if 1
      // Fast version.
      return ((constants.x ^ key) + constants.y) % kPrimeDivisor;
#else
      // Slow version.
      return ((unsigned long long)constants.x * key + constants.y) %
             kPrimeDivisor;
#endif
    }

    //! Computes the value of a hash function for a given key.
    /*! \param[in] functions        All of the constants used by the hash
      functions.
      ! \param[in] which_function   Which hash function is being used.
      ! \param[in] key              Key being hashed.
      ! \returns                    The value of a hash function with a given
      key.
     */
    template <unsigned kNumHashFunctions>
    inline __device__ __host__ unsigned hash_function(
        const Functions<kNumHashFunctions> functions,
