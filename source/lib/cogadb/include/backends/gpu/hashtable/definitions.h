...
namespace CudaHT {
  namespace CuckooHashing {
    ...
    typedef unsigned long long Entry;  //!< A key and its value are stored in a
    // 64-bit number.  The key is stored in
    // the upper 32 bits.

    const unsigned kMaxRestartAttempts = 10;  //!< Number of build attempts.
    const unsigned kKeyEmpty =
        0xffffffffu;  //!< Signifies empty slots in the table.
