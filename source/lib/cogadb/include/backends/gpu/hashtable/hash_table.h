...
namespace CudaHT {
  namespace CuckooHashing {
    ...
    //! Compute how many thread blocks are required for the given number of
    // threads.
    dim3 ComputeGridDim(unsigned threads);

    //! Compute how long an eviction chain is allowed to become for a given
    // input size.
    ...
    class HashTable {
     public:
      ...
      /*! @param[in] max_input_size   Largest expected number of items in the
       *input.
       ...
       *input
       *                          keys.
       *  @param[in] d_vals       Device memory array containing the keys'
       *values.
       *  @returns Whether the hash table was built successfully (true) or
       *           not (false).
       *
       *  Several attempts are allowed to build the hash table in case of
       *failure.
       *  The input keys are expected to be completely unique.
       *  To reduce the chance of a failure, increase the space usage or number
       *of
       *  functions.
      ...
      //! Query the hash table for a relational join.
      /*! @param[in] n_queries         Number of keys in the query set.
       *  @param[in] d_query_keys      Device memory array containing all of
       *                               the query keys.
       *  @param[in] d_out_query      Values for the query keys.
       *  @param[in] d_out_table      Values for the table keys.
       *                              Computes Semi-Join if Input is NULL
       *
       *
       *  kNotFound is returned for any query key that failed to be found
       *  in the table.
       */
      virtual bool RetrieveJoin(const unsigned n_queries,
                                const unsigned *d_query_keys,
                                unsigned &result_size, unsigned *d_out_query,
                                unsigned *d_out_table);

      virtual bool RetrieveSemiJoin(const unsigned n_queries,
                                    const unsigned *d_query_keys,
                                    unsigned &result_size,
                                    unsigned *d_out_query, bool anti = false);

      virtual bool RetrieveBitmapSemiJoin(const unsigned n_queries,
                                          const unsigned *d_query_keys,
                                          char *d_flag_array,
                                          bool anti = false);
      ...
      // inline void setTheCudpp(CUDPPHandle theCudpp_)     { theCudpp =
      // theCudpp_; }
     ...
     protected:
      ...
      // join temporary data
      unsigned *d_table_temp;
      unsigned table_temp_size;
      unsigned *d_query_temp;
      unsigned query_temp_size;
    ...
    namespace CUDAWrapper {
      ...
      void ClearTable(const unsigned slots_in_table, const Entry fill_value,
                      Entry *d_array, cudaStream_t computeStream);

      //! Calls the Cuckoo Hash construction kernel.
      cudaError_t CallCuckooHash(
          const unsigned n_entries, const unsigned num_hash_functions,
          const unsigned *d_keys, const unsigned *d_values,
          const unsigned table_size, const Functions<2> constants_2,
          const Functions<3> constants_3, const Functions<4> constants_4,
          const Functions<5> constants_5, const unsigned max_iteration_attempts,
          Entry *d_contents, uint2 stash_constants, unsigned *d_stash_count,
      ...
      //! Calls the kernel that performs retrieval of hash joins.
      bool CallRetrieveHashJoin(
          const unsigned n_queries, const unsigned num_hash_functions,
          const unsigned *keys_in, const unsigned table_size,
          const Entry *table, const Functions<2> constants_2,
          const Functions<3> constants_3, const Functions<4> constants_4,
          const Functions<5> constants_5, const uint2 stash_constants,
          const unsigned stash_count, unsigned &result_size,
          unsigned *d_query_temp, unsigned *d_table_temp, unsigned *d_query_out,
          unsigned *d_table_out, bool anti, cudaStream_t computeStream);
      ...
      bool CallBitmapSemiJoin(
          ...
          const Functions<3> constants_3, const Functions<4> constants_4,
          const Functions<5> constants_5, const uint2 stash_constants,
          const unsigned stash_count, char *d_flag_array, bool anti,
          cudaStream_t computeStream);
    };
    /// @}
