...
namespace CoGaDB {
  ...
  class BitPackedDictionaryCompressedCol {
   public:
  ...
  class BitPackedDictionaryCompressedColumn
      : public CompressedColumn<ValueType>,
        public BitPackedDictionaryCompressedCol {
   public:
    typedef std::map<ValueType, uint32_t> Dictionary;

    /***************** constructors and destructor *****************/
    BitPackedDictionaryCompressedColumn(
    ...
    virtual ~BitPackedDictionaryCompressedColumn();
  ...
  template <typename ValueType>
  const typename BitPackedDictionaryCompressedColumn<ValueType>::Dictionary &
  BitPackedDictionaryCompressedColumn<ValueType>::getDictionary() const
      throw() {
    return dictionary_;
  }
  ...
  BitPackedDictionaryCompressedColumn<ValueType>::getCompressedValues() const
      throw() {
    return data_items_;
  }
   ...
   */
  template <typename ValueType>
  std::pair<bool, uint32_t> BitPackedDictionaryCompressedColumn<
  ...
  template <typename ValueType>
  unsigned int BitPackedDictionaryCompressedColumn<
      ValueType>::getNumberofDistinctValues() const {
    return this->dictionary_.size();
  }

  template <typename ValueType>
  uint32_t BitPackedDictionaryCompressedColumn<ValueType>::getLargestID()
      const {
    return this->maximal_id_;
  }

  template <typename ValueType>
  uint64_t BitPackedDictionaryCompressedColumn<ValueType>::getNumberOfRows()
      const {
  ...
  template <typename ValueType>
  BitPackedDictionaryCompressedColumn<ValueType>::
      BitPackedDictionaryCompressedColumn(const std::string &name,
  ...
  template <typename ValueType>
  BitPackedDictionaryCompressedColumn<
      ValueType>::~BitPackedDictionaryCompressedColumn() {}

  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::insert(
  ...
  template <typename ValueType>
  const boost::any BitPackedDictionaryCompressedColumn<ValueType>::get(
      TID tid) {
    return boost::any(this->operator[](tid));
  }

  template <typename ValueType>
  void BitPackedDictionaryCompressedColumn<ValueType>::print() const throw() {
    ...
    for (unsigned int i = 0; i < this->number_of_rows_; i++) {
      ...
      int y = x >> (((std::min((uint64_t) this->number_of_values_per_word_,
                               (this->number_of_rows_ -
  ...
  template <typename ValueType>
  size_t BitPackedDictionaryCompressedColumn<ValueType>::size() const throw() {
    return getNumberOfRows();  // data_items_.size();
  }

  template <typename ValueType>
  const ColumnPtr BitPackedDictionaryCompressedColumn<ValueType>::copy() const {
    return ColumnPtr(new BitPackedDictionaryCompressedColumn<ValueType>(*this));
  }

  template <typename ValueType>
  const ColumnPtr BitPackedDictionaryCompressedColumn<ValueType>::gather(
    ...
    BitPackedDictionaryCompressedColumn<ValueType> *result =
        new BitPackedDictionaryCompressedColumn<ValueType>(
            this->name_, this->db_type_, this->number_of_bits_per_value_);
  ...
  inline uint32_t getUncompressedID(TID index,
                                    const std::vector<uint32_t> &data_items_,
                                    int32_t number_of_values_per_word_,
                                    int32_t number_of_bits_per_value_,
                                    uint64_t number_of_rows_) {
    // retrieve word containing compressed data
    uint32_t x = data_items_[index / number_of_values_per_word_];
    // which entry is it in the word
    int offset = index % number_of_values_per_word_;
    ...
    uint64_t highestOffsetInWord =
        std::min((uint64_t)number_of_values_per_word_,
                 (number_of_rows_ -
                  (index / number_of_values_per_word_) *
  ...
  BitPackedDictionaryCompressedColumn<T>::createColumnGroupingKeys(
      const ProcessorSpecification &proc_spec) const {
  ...
  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::update(
    ...
    // determine highest offset within in word
    uint64_t highestOffsetInWord =
        std::min((uint64_t) this->number_of_values_per_word_,
                 (this->number_of_rows_ -
   ...
   */
  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::remove(TID tid) {
    int index = tid;
    // retrieve word containing compressed data
    uint32_t x = this->data_items_[index / this->number_of_values_per_word_];
    // which entry is it in the word
  ...
  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::remove(
  ...
  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::clearContent() {
  ...
  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::store_impl(
      const std::string &path_to_table_dir,
      boost::archive::binary_oarchive &oa) {
    oa << this->data_items_;
    oa << this->dictionary_;
    oa << this->reverse_lookup_vector_;
    oa << this->maximal_id_;
  ...
  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::load_impl(
    ...
    this->number_of_values_per_word_ = 32 / this->number_of_bits_per_value_;
    this->padding_ = 32 % this->number_of_bits_per_value_;
    ...
    std::cout << "Computed number of rows: "
              ...
              << std::endl;
    // make sure that we have at most number of rows as can be stored
    assert(this->number_of_rows_ <=
           this->data_items_.size() * this->number_of_values_per_word_);
    return true;
  }

  template <typename ValueType>
  bool BitPackedDictionaryCompressedColumn<ValueType>::isCompressed() const
      throw() {
    return true;
  }

  template <typename ValueType>
  ValueType &BitPackedDictionaryCompressedColumn<ValueType>::operator[](
      const TID index) {
    // retrieve word containing compressed data
    uint32_t x = this->data_items_[index / this->number_of_values_per_word_];
    // which entry is it in the word
  ...
  template <typename ValueType>
  size_t BitPackedDictionaryCompressedColumn<ValueType>::getSizeinBytes() const
      throw() {
    // std::vector<uint32_t> data_items_
  ...
  template <>
  inline size_t
  BitPackedDictionaryCompressedColumn<std::string>::getSizeinBytes() const
      throw() {
