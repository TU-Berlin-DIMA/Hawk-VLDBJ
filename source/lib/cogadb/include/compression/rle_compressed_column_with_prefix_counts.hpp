...
namespace CoGaDB {
  ...
  template <class T>
  class RLECompressedColumnWithPrefixCounts : public CompressedColumn<T> {
   public:
    RLECompressedColumnWithPrefixCounts(const std::string &name,
  ...
  template <typename T>
  template <typename InputIterator>
  bool RLECompressedColumnWithPrefixCounts<T>::insert(InputIterator first,
                                                      InputIterator last) {
