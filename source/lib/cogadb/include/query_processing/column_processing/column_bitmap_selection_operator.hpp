...
namespace CoGaDB {
  namespace query_processing {
    namespace physical_operator {
      ...
      class CPU_column_bitmap_selection_operator
       ...
       public:
        ...
        CPU_column_bitmap_selection_operator(
            const hype::SchedulingDecision& sched_dec, TypedOperatorPtr child,
            const Predicate& pred);
        virtual bool execute();
        virtual ~CPU_column_bitmap_selection_operator();

       private:
        Predicate pred_;
      };

      class GPU_column_bitmap_selection_operator
          : public hype::queryprocessing::UnaryOperator<ColumnPtr, ColumnPtr>,
            BitmapOperator {
       public:
        typedef column_processing::cpu::TypedOperatorPtr TypedOperatorPtr;
        GPU_column_bitmap_selection_operator(
            const hype::SchedulingDecision& sched_dec, TypedOperatorPtr child,
            const Predicate& pred);
        virtual bool execute();
        virtual ~GPU_column_bitmap_selection_operator();
    ...
    namespace logical_operator {
      ...
      class Logical_Column_Bitmap_Selection
          : public hype::queryprocessing::TypedNode_Impl<
                ColumnPtr,
                physical_operator::
                    map_init_function_column_bitmap_selection_operator>  // init_function_column_bitmap_selection_operator>
      // //init_function_column_bitmap_selection_operator>
      {
       public:
        Logical_Column_Bitmap_Selection(
            const Predicate&,
