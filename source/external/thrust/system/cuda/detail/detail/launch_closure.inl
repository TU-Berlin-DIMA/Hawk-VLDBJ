...
/*BEGIN THRUST MODIFICATION BY COGADB TEAM*/
namespace CoGaDB{
   namespace gpu {
      cudaStream_t* getCUDAStream();
   };
};
/*END THRUST MODIFICATION BY COGADB TEAM*/
  ...
  struct closure_launcher_base
{
  ...
  static void launch(Closure f, Size1 num_blocks, Size2 block_size, Size3 smem_size)
  {
    ...
    {
/*BEGIN THRUST MODIFICATION BY COGADB TEAM*/
      cudaStream_t* stream = CoGaDB::gpu::getCUDAStream();
      launch_closure_by_value<<<(unsigned int) num_blocks, (unsigned int) block_size, (unsigned int) smem_size,*stream>>>(f);
