function compute_statistics_of_measurement_file {
   ...
   python3 compute_average.py "$measurement_file" >> results/$f3
}
...
echo "C-code execution strategy c_code.coga"
cat variants/c_code/c_code.coga > c_code.coga
     ...
     while [ ! -e finished ]; do
         ...
         if [ $RET -ne 0 ]; then
             ...
             if [ $NUM_TRIES -lt 3 ]; then
 	         ...
 	         bash ../send_error_email.sh "Experiments Failed For Variant: $v"
                 exit -1
             fi
         else
