...
for run_type in "cpu" "any"; do
	...
	rm -f experimental_result_data/measurements_$run_type.csv
	#echo -e "ID\tscale factor\tquery name\tnumber of users\tnumber of queries\tnumber of CPUs\tnumber of GPUs\tNUMBER_OF_EXECUTED_GPU_OPERATORS\tNUMBER_OF_ABORTED_GPU_OPERATORS\tWORKLOAD_EXECUTION_TIME-(ms)\tTOTAL_CPU_TIME-(ms)\tWASTED_TIME_DUE_TO_ABORTED_GPU_OPERATORS-(ns)\tMINIMAL QUERY EXECUTION TIME-(ms)\tMAXIMAL QUERY EXECUTION TIME-(ms)\tAVERAGE QUERY EXECUTION TIME-(ms)\tVARIANCE IN QUERY EXECUTION TIME" >> experimental_result_data/measurements_$run_type.csv
	...
	for i in *_$run_type*.log; do
		...
		NUMBER_OF_CPUS=`cat $i | grep "NUMBER_OF_VIRTUAL_CPUS" | awk 'BEGIN{FS=":";}{print $2}'`
		NUMBER_OF_GPUS=`cat $i | grep "NUMBER_OF_VIRTUAL_GPUS" | awk 'BEGIN{FS=":";}{print $2}'`
...
#		echo -e "$COUNTER\t$SCALE_FACTOR\t$QUERY_NAME\t$NUMBER_OF_USERS\t$NUMBER_OF_QUERIES\t$NUMBER_OF_CPUS\t$NUMBER_OF_GPUS\t$NUMBER_OF_EXECUTED_GPU_OPERATORS\t$NUMBER_OF_ABORTED_GPU_OPERATORS\t$WORKLOAD_EXECUTION_TIME\t$TOTAL_CPU_TIME\t$WASTED_TIME_DUE_TO_ABORTED_GPU_OPERATORS\t$QUERY_EXECUTION_STATISTICS" >> measurements_$run_type.csv

#	rm -f experimental_result_data/measurements_$run_type.csv
#	echo -e "ID\tscale factor\tquery name\tnumber of users\tnumber of queries\tNUMBER_OF_EXECUTED_GPU_OPERATORS\tNUMBER_OF_ABORTED_GPU_OPERATORS\tWORKLOAD_EXECUTION_TIME-(ms)\tWASTED_TIME_DUE_TO_ABORTED_GPU_OPERATORS-(ns)\tMINIMAL QUERY EXECUTION TIME-(ms)\tMAXIMAL QUERY EXECUTION TIME-(ms)\tAVERAGE QUERY EXECUTION TIME-(ms)\tVARIANCE IN QUERY EXECUTION TIME" >> experimental_result_data/measurements_$run_type.csv
#	cd experimental_result_data
#	COUNTER=0
#	for i in *_$run_type*.log; do
...
#		#compute minimal, maximal, and average query execution time in one run as well as the variance in execution time
#		#cat "$i" | grep "Execution Time" | awk '{print $3}' | sed -e 's/\./,/g' -e 's/,.*//g' > tmp
#		cat "$QUERY_RUNTIMES" | grep "$QUERY_NAME" | awk '{print $2}' | sed -e 's/\./,/g' -e 's/,.*//g' -e 's/ms//g' > tmp
#		QUERY_EXECUTION_STATISTICS=`cat tmp | awk -f ../compute_average.awk`
#		rm tmp
#		
#		echo -e "$COUNTER\t$SCALE_FACTOR\t$QUERY_NAME\t$NUMBER_OF_USERS\t$NUMBER_OF_QUERIES\t$NUMBER_OF_EXECUTED_GPU_OPERATORS\t$NUMBER_OF_ABORTED_GPU_OPERATORS\t$WORKLOAD_EXECUTION_TIME\t$WASTED_TIME_DUE_TO_ABORTED_GPU_OPERATORS\t$QUERY_EXECUTION_STATISTICS" >> measurements_$run_type.csv
